...
<a id="102"></a>
### 1.0.2 A restriction is a rule or effect that states something cannot happen. A requirement is a rule or effect that states that something should happen if possible. An allowance is a rule or effect that states something can happen. A restriction takes precedence over any requirement or allowance, and a requirement takes precedence over any allowance, subject to [1.0.1a](#101a).
...
<a id="112"></a>
### 1.1.2 A player's hero is a hero-card.[1.3.2a](#132a)
...
<a id="113"></a>
### 1.1.3 A player's card-pool is a collection of deck-cards[1.3.2c](#132c) and arena-cards.[1.3.2d](#132d) A card can only be included in a player's card-pool if the card's supertypes are a subset of their hero's supertypes.[2.11](#211)
...
<a id="113a"></a>
#### 1.1.3a If an effect allows a player to start the game with one or more cards with supertypes that are not a subset of their hero's supertypes, those cards may be included in the player's card-pool as long as they start the game as specified by the effect.[4.1.6c](#416c) Card text of meta-static abilities refers to the player's card-pool as the player's "deck."

<a id="113b"></a>
#### 1.1.3b A hybrid card may be included in a player's card-pool if either of the hybrid card's supertype sets is a subset of their hero's supertypes.[2.14.1b](#2141b)
...
<a id="115"></a>
### 1.1.5 In a game, a player's opponents include all other players who are not in their party.[1.1.4](#114)
...
<a id="121"></a>
### 1.2.1 An object is an element of the game with properties[2] and located in a zone[3] or a player's inventory.[4.1.6](#416) Cards, attacks, macros, and layers are objects.
...
<a id="122b"></a>
#### 1.2.2b An object with a name property and/or moniker has the object identity of that name and/or moniker.[2.7](#27)
...
<a id="122d"></a>
#### 1.2.2d An attack-card, attack-proxy, or attack-layer has the object identity "attack."[1.4](#14)
...
<a id="122f"></a>
#### 1.2.2f A permanent has the object identity "permanent."[1.3.3](#133)
...
<a id="132b"></a>
#### 1.3.2b A token-card is any card with the type token. A token-card is not considered part of a player's card-pool.[1.1.3](#113)
...
<a id="143"></a>
### 1.4.3 An attack-proxy is a non-card object with the attack ability[8.3.1](#831) that represents the attack of another object (attack-source).
...
<a id="143a"></a>
#### 1.4.3a An attack-proxy is a separate object that acts as an extension of its attack-source.[1.4.3b](#143b) It can only be referenced by effects using the object identity "attack",[1.2.2](#122) but is considered to inherit the properties of its attack-source in addition to any existing properties it has, with the exception of the activated and resolution abilities of its attack-source. An attack-proxy is not a copy of its source and does not have a copy of its source's properties.
...
<a id="144"></a>
### 1.4.4 An attack-layer is a layer with the attack effect[8.5.38](#8538) that represents an attack with no properties on the stack.
...
<a id="152"></a>
### 1.5.2 A macro cannot be and is not considered part of a player's card-pool.[1.1.3](#113)
...
<a id="161"></a>
### 1.6.1 A layer is an object on the stack[3.15](#315) that is yet to be resolved. Card-layers, activated-layers, and triggered-layers are layers.
...
<a id="162b"></a>
#### 1.6.2b An activated-layer is a layer created by an activated ability. An activated-layer is created on the stack, and then can only exist on the stack.[5.2](#52)
...
<a id="162c"></a>
#### 1.6.2c A triggered-layer is a layer created by a triggered effect. A triggered-layer is created before it is put on the stack, and then can only exist on the stack.[5.5](#55)
...
<a id="171"></a>
### 1.7.1 An ability is a property of an object that influences the game by generating effects or by creating a layer on the stack that resolves and generates effects. The base abilities of a non-token card are determined by its rules text.[2.12](#212) The base abilities of a token card, macro, or non-card layer are defined by a rule, or the effect or ability that created it.
...
<a id="173a"></a>
#### 1.7.3a Activated abilities can be activated by a player to put an activated-layer on the stack.[5.2](#52)

<a id="173b"></a>
#### 1.7.3b Resolution abilities generate effects when a layer with the ability resolves on the stack.[5.3](#53)

<a id="173c"></a>
#### 1.7.3c Static abilities simply generate effects.[5.4](#54)
...
<a id="174b"></a>
#### 1.7.4b An activated ability is functional if its cost can only be paid, or it explicitly specifies it can be activated, when the source is private or outside the arena, and the source meets that condition.[5.2.4](#524)
...
<a id="174c"></a>
#### 1.7.4c A resolution ability is functional when its source object resolves as a layer on the stack, otherwise, it is non-functional.[5.3](#53)
...
<a id="174d"></a>
#### 1.7.4d A meta-static ability is functional outside the game.[5.4.3](#543)
...
<a id="174e"></a>
#### 1.7.4e A play-static ability is functional when its source is public in any zone and when it is played.[5.4.4](#544)
...
<a id="174f"></a>
#### 1.7.4f A property-static ability is functional when its source is in any zone or outside the game.[5.4.5](#545)
...
<a id="174g"></a>
#### 1.7.4g A while-static ability is functional when its while-condition is met.[5.4.7](#547)
...
<a id="175a"></a>
#### 1.7.5a Modes of the source are declared as the source is added as a layer on the stack.[5.1.4](#514)[6.6.6a](#666a)
...
<a id="175c"></a>
#### 1.7.5c If the same mode is selected more than once, the modes are considered separate and do not create a compound event.[1.9.2](#192) If the mode contains a targeted effect, the same target(s) may be selected for each time the mode is selected.
...
<a id="183b"></a>
#### 1.8.3b If any of the optional effects cannot be generated due to a rule or effect, cannot resolve successfully based on the current game state, or contain an asset-cost which cannot be paid,[1.14.2](#1142) the effect cannot be generated/applied and the player cannot choose to generate/apply the effect. If the optional effects are phrased with "may choose to" then the player may choose to generate the effects regardless of their outcome.
...
<a id="185d"></a>
#### 1.8.5d If the targeted effect can only be applied to certain objects, the otherwise legal targets are restricted based on the requirements of the parameters for the effect.[8.5](#85)
...
<a id="186a"></a>
#### 1.8.6a If two or more players are instructed to determine the parameters of an effect, those players determine the parameters in clockwise order, starting with the controller of the effect.[1.1.6](#116)
...
<a id="186b"></a>
#### 1.8.6b If the parameters of an effect include determining an object, the player can only determine public objects in the arena or on the stack as parameters for the effect, unless stated otherwise.[3.0.3](#303) If the parameters of effect include determining a object in a public zone, the player can only determine public objects in that zone, unless stated otherwise.
...
<a id="186d"></a>
#### 1.8.6d If an effect would generate a compound event, the player determines the order of the individual events.[1.9.2](#192)
...
<a id="192c"></a>
#### 1.9.2c If an event involves two or more players performing an instruction, it is a compound event where each player performs that instruction in clockwise order as an individual event, starting with the turn-player.[1.1.6](#116) If the event was produced by an effect, each player performs that instruction starting with the controller of the effect instead.
...
<a id="193"></a>
### 1.9.3 A composite event is an event that is made up of one or more internal events. An effect that produces a composite event typically uses an effect keyword.[8.5](#85)
...
<a id="1101"></a>
### 1.10.1 A game state is a moment in the game. The game transitions between states when an event occurs.[1.9](#19) A priority state is a game state where a player would typically receive priority.

<a id="1102"></a>
### 1.10.2 When the game transitions to a new priority state, the following game state actions are performed first:

<a id="1102a"></a>
#### 1.10.2a First, if one or more heroes have died, their player loses the game (or the game ends in a draw).[4.5](#45)

<a id="1102b"></a>
#### 1.10.2b Second, if one or more living objects in the arena have 0 life total, they are cleared simultaneously as a single event. Living objects that cease to exist are considered to have died.[2.5.1a](#251a)[3.0.12](#3012)

<a id="1102c"></a>
#### 1.10.2c Third, if a continuous effect allows a player to look at a card based on location, they may start looking at that card until it is no longer in that location.[8.5.11](#8511)

<a id="1102d"></a>
#### 1.10.2d Forth, if the state-condition of a state-based triggered effect is met, the effect triggers. Then, if any triggered-layers have been created, but have not been added to the stack, they are added to the stack. If two or more triggered-layers have been created, then each player in clockwise-order adds their own triggered-layers to the stack, starting with a player chosen by the turn-player.[6.6.6b](#666b)

<a id="1102e"></a>
#### 1.10.2e Fifth and finally, if the combat chain is open and a rule or effect has closed the combat chain, the Close Step of combat begins.[7.7.2](#772)
...
<a id="1113"></a>
### 1.11.3 The Action Phase is the only phase when players get priority. Within the action phase, players do not get priority during the Close Step of combat. At the beginning of the action phase, during most steps of combat, and after the resolution of a layer, the turn-player gains priority.[4.2](#42)[7][5.3](#53)

<a id="1114"></a>
### 1.11.4 The active player may pass priority to the next player, referred to as "pass."

<a id="1114a"></a>
#### 1.11.4a If a player passes, priority is given to the next player in clockwise order.[1.1.6](#116) Typically, if all players pass in succession without playing any cards or activating any abilities, and the stack is not empty, the top layer of the stack resolves[5.3](#53) - otherwise if the stack is empty, the phase or step ends.[4][7]

<a id="1115"></a>
### 1.11.5 If the active player plays a card or activates an ability, they regain priority after the card has been played or the ability has been activated.[5.1.10](#5110) If the active player passes, they lose priority until they receive it again from a rule. No player has priority while playing a card, activating an ability, resolving a layer, during a game process, and/or during game state actions.
...
<a id="1124d"></a>
#### 1.12.4d The power symbol is {p} and represents a power value. It is also used to refer to physical damage.[8.5.3b](#853b)
...
<a id="1124g"></a>
#### 1.12.4g The tap symbol is {t} and represents the tap effect.[8.5.55](#8555)

<a id="1124h"></a>
#### 1.12.4h The untap symbol is {u} and represents a untap effect.[8.5.56](#8556)
...
<a id="1132a"></a>
#### 1.13.2a A player can gain action points during their action phase from the following: at the start of their action phase,[4.3](#43) the ability go again,[8.3.5](#835) and effects that grant the player action points.
...
<a id="1133a"></a>
#### 1.13.3a A player can gain resource points from the following: pitching cards during the payment of an asset-cost that requires resource points,[1.14.3](#1143) and effects that grant the player resource points.
...
<a id="1135a"></a>
#### 1.13.5a A player can gain chi points from the following: pitching cards during the payment of an asset-cost that requires chi or resource points.[1.14.3](#1143)
...
<a id="1142b"></a>
#### 1.14.2b If a player does not have enough assets and does not gain any additional assets to pay an asset-cost, the cost cannot be paid and the game state is reversed to before the cost is paid. If the asset-cost is mandatory and part of another action, the entire action is reversed..[1.10.3](#1103)

<a id="1142c"></a>
#### 1.14.2c To pay a chi point cost, the player must use chi points.[1.13.5](#1135) If they have fewer chi points than required, the player may pitch cards[1.14.3](#1143) one at a time until they have enough chi points to pay the cost.

<a id="1142d"></a>
#### 1.14.2d To pay a resource point cost, the player must use resource points[1.13.3](#1133) and chi points.[1.13.5](#1135) If they have fewer resource points and chi points than required, the player may pitch cards[1.14.3](#1143) one at a time until they have enough resource points and chi points to pay the cost, then the cost is paid. The player must use all of their chi points before using any of their resource points to pay a resource point cost.
...
<a id="1142e"></a>
#### 1.14.2e To pay a life point cost, the player must use life points.[1.13.4](#1134)

<a id="1142f"></a>
#### 1.14.2f To pay an action point cost, the player must use action points.[1.13.2](#1132)

<a id="1143"></a>
### 1.14.3 To pitch a card as a player, the player moves the card from their hand to the pitch zone and gains assets. The pitch property of a card determines what type and how many assets a player gains when the card is pitched.[2.8](#28)

<a id="1143a"></a>
#### 1.14.3a A card cannot be pitched if it does not have the pitch property.

<a id="1143b"></a>
#### 1.14.3b A player may only pitch a card if that card will gain them assets they need to pay an asset-cost or if they are instructed by an effect.[8.5.44](#8544)
...
<a id="1144b"></a>
#### 1.14.4b Before the effect-cost is paid, if any of the effects cannot be generated due to a rule or effect, or the effects cannot resolve successfully based on the current game state, the cost cannot be paid and the game state is reversed to before the cost is paid. If the effect-cost is mandatory and part of another action, the entire action is reversed..[1.10.3](#1103)
...
<a id="1151"></a>
### 1.15.1 A counter is a physical marker placed on any public object.[3.0.3](#303) A counter is not an object and does not have properties. The identity of a counter is defined by its name, or its numerical value and symbol. Counters with the same name, or value and symbol, are functionally identical and interchangeable.

<a id="1152"></a>
### 1.15.2 When a counter is on an object, it modifies its properties and/or interacts with effects.

<a id="1152a"></a>
#### 1.15.2a A counter with a numerical value and symbol that corresponds to an object property modifies that property of the object it is on. This modification is considered a rule, not an effect, but does take place at the same time as continuous effects that modify the object in the same way.[6.3.2](#632)[6.3.3](#633)

<a id="1153"></a>
### 1.15.3 When an object ceases to exist, the counters on that object cease to exist.[3.0.9](#309) When a counter is removed from an object it ceases to exist.
...
<a id="201"></a>
### 2.0.1 A property is an attribute of an object that defines how the object interacts with the rules and effects of the game. There are 13 properties an object may have: abilities,[1.7](#17) color strip, cost, defense, intellect, life, name, pitch, power, subtypes, supertypes, text box, and type.
...
<a id="221"></a>
### 2.2.1 Cost is a numeric property of a card or ability, which determines the starting resource asset-cost to play the card or activate the ability.[5.1.6](#516)
...
<a id="231"></a>
### 2.3.1 Defense is a numeric property of an object, which represents the value contributed to the total sum of defense used in the damage step of combat.[7.5](#75)
...
<a id="241"></a>
### 2.4.1 Intellect is a numeric property of a hero card, which represents the number of cards the controlling player draws up to at the end of their turn.[4.4](#44)
...
<a id="251a"></a>
#### 2.5.1a A permanent with the life property is a living object.[1.3.3](#133)
...
<a id="253b"></a>
#### 2.5.3b Life gained and life lost are not continuous effects - they are discrete effects that apply once, and they permanently modify the life total.[8.5.7](#857)
...
<a id="253f"></a>
#### 2.5.3f If a permanent's life total is reduced to zero, it is cleared as a game state action; or if the permanent is a hero, their player loses or the game is a draw as a game state action.[1.10.2](#1102)[4.5](#45)
...
<a id="271"></a>
### 2.7.1 Name is a property of an object, which represents one of its object identities and determines the object's uniqueness (along with the pitch property).[1.2.2](#122)[1.3.4](#134)
...
<a id="281"></a>
### 2.8.1 Pitch is a property of a card, which represents the assets a player gains when they pitch the card. The pitch value of the card is the number of assets gained when pitched and it determines the object's uniqueness (along with the name property).[1.14.3](#1143)[1.3.4](#134)
...
<a id="291"></a>
### 2.9.1 Power is a numeric property of an object, which represents the power value used in the damage step of combat.[7.5](#75)
...
<a id="2106"></a>
### 2.10.6 Subtypes are either functional or non-functional keywords. Functional subtypes add additional rules to an object.[8.2](#82) Non-functional subtypes do not add additional rules to an object.

<a id="2106a"></a>
#### 2.10.6a The functional subtype keywords are (1H), (2H), Affliction, Ally, Arrow, Ash, Attack, Aura, Construct, Figment, Invocation, Item, Landmark, Off-Hand, and Quiver.[8.2](#82)
...
<a id="2123b"></a>
#### 2.12.3b If the rules text specifies the name and/or moniker of another object in the context of creating it, it refers to a hypothetical object with defined properties, including that name.[8.6](#86) Otherwise, if the rules text specifies the name and/or moniker of another object it refers to any existing object with that name and/or moniker.

<a id="213"></a>
## 2.13 Traits

<a id="2131"></a>
### 2.13.1 Trait is a property of an object, which represents one of its object identities.[1.2.2](#122)
...
<a id="2154a"></a>
#### 2.15.4a The types of an activated ability layer include the types determined by the activated ability.[5.2.1](#521)

<a id="2155"></a>
### 2.15.5 An object can gain or lose types from rules and/or effects.

<a id="2156"></a>
### 2.15.6 Types are functional keywords and add additional rules to an object.[2.15](#215)

<a id="2156a"></a>
#### 2.15.6a The type keywords are Action, Attack Reaction, Block, Companion, Defense Reaction, Demi-Hero, Equipment, Hero, Instant, Macro, Mentor, Resource, Token, and Weapon.[8.1](#81)
...
<a id="3014b"></a>
#### 3.0.14b A sub-card is not considered to be in the arena, even if its top-card is in the arena. If a card becomes a sub-card, it ceases to exist and becomes a new card with no relation to its previous existence.[3.0.9](#309)
...
<a id="3014e"></a>
#### 3.0.14e If a top-card ceases to exist, its sub-cards are cleared as part of the same event.[3.0.12](#3012)
...
<a id="322"></a>
### 3.2.2 An arms zone can only contain up to one object which is equipped to that zone.[8.5.41](#8541)

<a id="322a"></a>
#### 3.2.2a An object can only be equipped to an arms zone if it has subtype arms.

<a id="323"></a>
### 3.2.3 A player may equip an arms card to their arms zone at the start of the game.[4.1](#41)
...
<a id="332"></a>
### 3.3.2 An arsenal zone can only contain up to one of its owner's deck-cards.[1.3.2c](#132c)
...
<a id="334"></a>
### 3.3.4 Cards in an arsenal zone may be played.[5.1](#51)
...
<a id="352"></a>
### 3.5.2 A chest zone can only contain up to one object which is equipped to that zone.[8.5.41](#8541)

<a id="352a"></a>
#### 3.5.2a An object can only be equipped to a chest zone if it has subtype chest.

<a id="353"></a>
### 3.5.3 A player may equip a chest card to their chest zone at the start of the game.[4.1](#41)
...
<a id="362"></a>
### 3.6.2 The combat chain zone can only contain cards and attack-proxies.[1.4.3](#143)
...
<a id="372"></a>
### 3.7.2 A deck zone can only contain its owner's deck-cards.[1.3.2c](#132c)
...
<a id="376"></a>
### 3.7.6 A player's starting deck starts the game in their deck zone.[4.1](#41)
...
<a id="392"></a>
### 3.9.2 A hand zone can only contain its owner's deck-cards.[1.3.2c](#132c)
...
<a id="3102"></a>
### 3.10.2 A head zone can only contain up to one object which is equipped to that zone.[8.5.41](#8541)

<a id="3102a"></a>
#### 3.10.2a An object can only be equipped to a head zone if it has subtype head.

<a id="3103"></a>
### 3.10.3 A player may equip a head card to their head zone at the start of the game.[4.1](#41)
...
<a id="3114"></a>
### 3.11.4 A player must start the game with their hero card in their hero zone.[4.1](#41)

<a id="3115"></a>
### 3.11.5 A hero's soul refers to the collection of sub-objects under the hero card.[3.0.14](#3014)
...
<a id="3122"></a>
### 3.12.2 A legs zone can only contain up to one object which is equipped to that zone.[8.5.41](#8541)

<a id="3122a"></a>
#### 3.12.2a An object can only be equipped to a legs zone if it has subtype legs.

<a id="3123"></a>
### 3.12.3 A player may equip a legs card to their legs zone at the start of the game.[4.1](#41)
...
<a id="3142"></a>
### 3.14.2 A pitch zone can only contain its owner's deck-cards.[1.3.2c](#132c)
...
<a id="3153"></a>
### 3.15.3 The stack contains an ordered collection of layers.[1.6](#16)
...
<a id="3162"></a>
### 3.16.2 A weapon zone can only contain up to one object which is equipped to that zone.[8.5.41](#8541)

<a id="3162a"></a>
#### 3.16.2a An object can only be equipped to a weapon zone if it has the type weapon or the subtype off-hand or quiver. An object with the subtype 2H must be equipped to two weapon zones.[8.2.2](#822)

<a id="3163"></a>
### 3.16.3 A player may equip a weapon card or an off-hand card to their weapon zone at the start of the game.[4.1](#41)
...
<a id="401"></a>
### 4.0.1 A game of Flesh and Blood is preceded by the start-of-game procedure and ends when a player wins or the game is a draw.[4.1](#41)[4.5](#45)
...
<a id="414"></a>
### 4.1.4 Third, each player selects arena-cards from their card-pool that they will start the game with.[1.3.2d](#132d)

<a id="414a"></a>
#### 4.1.4a A player may select up to one arena-card for each of their arms, chest, head, legs, and weapon zones. Each card will start the game equipped in its respective zone, based on its type and/or subtype.[4.1.8](#418)
...
<a id="416"></a>
### 4.1.6 Fifth, all other cards in a player's card-pool that were not selected during [4.1.4](#414) or [4.1.5](#415) become that player's inventory.
...
<a id="417a"></a>
#### 4.1.7a After a player has presented their starting deck to be shuffled and/or cut, it is placed in their deck zone and they may no longer change any cards they have chosen in .[4.1.4](#414)[4.1.5](#415)
...
<a id="418"></a>
### 4.1.8 Seventh, each player in clockwise order equips their respective weapons and equipment, starting with the first turn-player .[1.1.6](#116)[8.5.41](#8541) Any other cards that start the game in a zone other than the deck zone are put in their respective zones. The "start of the game" event occurs and effects that trigger at the start of the game are triggered. Layers on the stack resolve and game state actions are performed as if all players are passing priority in succession until the stack is empty.[1.10.2](#1102)

<a id="418a"></a>
#### 4.1.8a If two or more triggered-layers are created, they are added to the stack in an order chosen by the first-turn-player.[6.6.6b](#666b)
...
<a id="442"></a>
### 4.4.2 First, the "beginning of the end phase" event occurs and effects that trigger at the beginning of the end phase are triggered. Layers on the stack resolve and game state actions are performed as if all players are passing priority in succession until the stack is empty..[1.10.2](#1102)
...
<a id="513a"></a>
#### 5.1.3a If the card has a variable cost that includes X, the player must declare the value of X.[1.12.2](#1122) If the player uses an effect to play the card without paying the cost that includes X, then the player must declare X as 0.
...
<a id="514a"></a>
#### 5.1.4a If the card has any modal resolution abilities, the player must declare the modes for those abilities.[1.7.5](#175) Then, if any resolution abilities generate effects that require a target, the player must declare all legal targets for those effects.[1.8.5](#185)

<a id="514b"></a>
#### 5.1.4b If the card is an attack, the player must declare the target(s) of the attack.[1.4.5](#145)

<a id="515"></a>
### 5.1.5 Check Legal Play: Fourth, the card is evaluated if it is legal to play before any costs are paid. If a rule or effect prevents the card from being played, or the parameters of the proposed card are illegal, the card is illegal to play and the game state is reversed to before the card was announced.[1.10.3](#1103)

<a id="516"></a>
### 5.1.6 Calculate Asset-Costs: Fifth, all asset-costs of the proposed card are calculated.[1.13](#113)

<a id="516a"></a>
#### 5.1.6a First, rules define the starting asset-cost. Second, effects that set an asset-cost are applied. Third, effects that increase an asset-cost are applied. Fourth and finally, effects that reduce an asset-cost are applied - if an effect would reduce an asset-cost to be less than zero, it instead reduces it to zero. If two or more effects are applied at each step, then they are applied in timestamp order.[6.3.4](#634)
...
<a id="517"></a>
### 5.1.7 Pay Asset-Costs: Sixth, the player pays all asset-costs (if any).[1.14.2](#1142)

<a id="517a"></a>
#### 5.1.7a If any asset-cost is not paid in full, the card is illegal to play and the game state is reversed to before the card is announced.[1.10.3](#1103)

<a id="518"></a>
### 5.1.8 Calculate Effect-Costs: Seventh, all effect-costs of the proposed cards are calculated.

<a id="518a"></a>
#### 5.1.8a If any effect-cost cannot be paid, based on the order declared by the player, the card is illegal to play and the game state is reversed to before the card was announced.[1.10.3](#1103)
...
<a id="522"></a>
### 5.2.2 To activate an activated ability is to create an activated-layer on the stack. A player can only activate an activated ability if they have priority, the ability is functional,[1.7.4](#174) and the player controls its source (or owns its source if there is no controller), unless otherwise specified by a rule or effect. Activating an activated ability involves the following:
...
<a id="522b"></a>
#### 5.2.2b The remainder of the process for activating an activated ability is identical to the process for playing a card listed in steps [5.1.3](#513)-,[5.1.10](#5110) substituting "play" for 'activate`, and "card" for "ability."
...
<a id="523c"></a>
#### 5.2.3c If an effect allows a source to attack additional times, it refers to activating an ability on the source with the attack ability ([8.3.1](#831)) or attack effect ([8.5.38](#8538)).
...
<a id="531"></a>
### 5.3.1 A resolution ability is an ability that generates effects when its source resolves as a layer on the stack. If the stack is not empty and all players pass in succession, the top layer of the stack resolves, except the Layer Step of combat .[7.1](#71) When a layer resolves it does so in the following order: Check resolution, Static effects, Layer effects, Go again, Leave stack, Clear.
...
<a id="533"></a>
### 5.3.3 Static effects: Second, static abilities of the layer become functional.[1.7.4](#174)
...
<a id="534a"></a>
#### 5.3.4a If the parameters of any effect are undetermined, those parameters are determined before the effect is generated.[1.8.6](#186)

<a id="534b"></a>
#### 5.3.4b If an effect fails, any remaining effects continue to be generated.[1.8.9](#189)

<a id="534c"></a>
#### 5.3.4c If the layer no longer exists after the generation of an effect, the last known information of the layer is used to determine the remainder of the resolution abilities and the effects that are generated.[1.2.3](#123)

<a id="534d"></a>
#### 5.3.4d If the layer is melded[8.3.38](#8338) and it is its first time being resolved, only the right-side resolution abilities generate their effects, then resolution stops and the turn-player gains priority; if it is the second time is being resolved, only the left-side resolution abilities generate their effects, then resolution continues.

<a id="535"></a>
### 5.3.5 Go again: Fourth, if the layer has go again, the controlling player gains 1 action point.[8.3.5](#835)

<a id="535a"></a>
#### 5.3.5a If the layer no longer exists, the last known information of the layer is used to determine whether the layer had go again.[1.2.3](#123)

<a id="536"></a>
### 5.3.6 Leave stack: Fifth, if a rule or effect would cause the layer to leave the stack, it does so.

<a id="536a"></a>
#### 5.3.6a A card-layer that becomes a permanent is moved to the arena.[8.2.4](#824)[8.2.13](#8213)[8.2.5](#825)[8.2.9](#829)

<a id="536b"></a>
#### 5.3.6b A card-layer with the type defense reaction becomes a defending card active chain link.[8.1.3](#813)

<a id="537"></a>
### 5.3.7 Clear: Sixth and finally, if the layer is still on the stack, it is cleared,[3.0.12](#3012) then the turn-player gains priority.
...
<a id="542"></a>
### 5.4.2 Static abilities that are functional generate static continuous effects.[6.2.3](#623)
...
<a id="546"></a>
### 5.4.6 A triggered-static ability is a static ability that generates a single triggered effect.[5.5](#55)
...
<a id="611"></a>
### 6.1.1 A discrete effect is an effect that changes the game state by producing an event.[1.9](#19) Discrete effects have no duration, and after the completion of their event, they have no further influence to the game state.
...
<a id="613"></a>
### 6.1.3 If a discrete effect is conditional,[1.8.4](#184) its condition is evaluated only once, at the time the effect would be generated. If the condition is met, the effect is generated - otherwise, it is not.
...
<a id="622d"></a>
#### 6.2.2d If a layer-continuous effect is conditional on being generated,[1.8.4](#184) its condition is evaluated only once, at the time the effect would be generated. If the condition is met, the effect is generated - otherwise, it is not. If the effect is conditional on being applied, the condition is still checked continuously.[1.8.4c](#184c)
...
<a id="623a"></a>
#### 6.2.3a A static-continuous effect starts as soon as the static ability that generated it becomes functional and ends when the static ability becomes non-functional.[1.7.4](#174)
...
<a id="623d"></a>
#### 6.2.3d If a static-continuous effect is conditional on being generated,[1.8.4](#184) its condition is evaluated at all times. The effect is generated when the condition is met and ceases to exist when the condition is not met.
...
<a id="625a"></a>
#### 6.2.5a If two or more effects would apply to an object at the same time, they are applied in the order determined staging..[6.3.1](#631)
...
<a id="632"></a>
### 6.3.2 Stage order is defined by how an effect applies to the object. Effects are grouped into stages and are applied in ascending stage order. If there are two or more effects in the same stage, the effects are applied using substage order.[6.3.3](#633)

Stage 1: Effects that modify copyable properties are applied.[8.5.25](#8525)
...
<a id="633a"></a>
#### 6.3.3a If there are two or more effects in the same substage, the effects are applied using timestamp order.[6.3.4](#634)
...
<a id="634a"></a>
#### 6.3.4a The timestamp of an effect is the moment it was generated by an ability. Layer-continuous effects are generated when their source resolves as a layer on the stack[6.2.2](#622) and static-continuous effects are generated when the static ability becomes functional.[6.2.3](#623)
...
<a id="661"></a>
### 6.6.1 A triggered effect is an effect that can be triggered to put a triggered-layer on the stack. Triggered effects are typically written in the format "[LIMIT?] (When / Whenever / At / The [ORDINAL] time / The next time) [EVENT and/or STATE] [ABILITIES]." Triggered effects never use the term "instead".[6.4](#64)
...
<a id="665b"></a>
#### 6.6.5b An event-based triggered effect only triggers if an event that meets the triggered condition occurs. If the event is modified before it occurs and no longer meets the triggered condition, the effect does not trigger.[6.4](#64) If the trigger condition also includes a state-trigger condition, that condition must also be met by the game state at the time the event occurs for the effect to trigger.
...
<a id="666"></a>
### 6.6.6 When a triggered-layer is created, it is added to the stack before the next player receives priority as a game state process.[1.10.2](#1102)

<a id="666a"></a>
#### 6.6.6a When a triggered-layer is added to the stack, the player that controls the effect must declare the parameters of all abilities of that layer. If the card has any modal resolution abilities, the player must declare the modes for those abilities.[1.7.5](#175) Then, if any resolution abilities generate effects that require a target, the player must declare all legal targets for those effects .[1.8.5](#185) If no legal targets can be declared, the triggered-layer ceases to exist and is not added to the stack.
...
<a id="701a"></a>
#### 7.0.1a During combat, while the combat chain is open, a player cannot play cards or activate activated abilities with the type action, except for attacks during the Resolution Step of combat.[7.6](#76) An action card/ability can still be played/activated as an instant during any step of combat when a player has priority.

<a id="702"></a>
### 7.0.2 The combat chain is a zone[3.6](#36) that is open during combat and closed otherwise. It comprises chain links when it is open, and is empty when it is closed.

<a id="702a"></a>
#### 7.0.2a The combat chain starts the game closed. If the combat chain is closed and an attack is added to the stack, the combat chain opens and the Layer Step begins immediately. The combat chain remains open until it is closed again.[7.7.2](#772)
...
<a id="703a"></a>
#### 7.0.3a A chain link is created when an attack is added to the combat chain as a chain link.[7.2.3](#723) The attack becomes the active-attack of chain link N+1, where N is the number of existing chain links on the combat chain.

<a id="703b"></a>
#### 7.0.3b The active chain link is the most recent chain link of the combat chain to be resolved. When a chain link is created during the Attack Step it remains the active chain link until it resolves[7.6.2](#762) or the combat chain closes.

<a id="703c"></a>
#### 7.0.3c When referring to a chain link as a collection, the properties, control, and ownership of a chain link are considered to be the same as the active-attack of that chain link. If the active-attack of a chain link ceases to exist, last known information about the active-attack is used to determine the properties, control, and ownership.[1.2.3](#123)
...
<a id="703e"></a>
#### 7.0.3e If there is an active chain link, any layer that is played/activated/triggered is considered to be played/activated/triggered on the active chain link for rules and effects.[7.0.3b](#703b)
...
<a id="704"></a>
### 7.0.4 An active-attack, is an attack that has been put onto the combat chain as a chain link[1.4](#14)[7.0.3a](#703a).
...
<a id="705"></a>
### 7.0.5 A defending card is a card that is designated as defending on a chain link for an attack-target by a rule or effect.[7.3.2](#732)[7.4.2d](#742d)[8.5.32](#8532)
...
<a id="705c"></a>
#### 7.0.5c A defending card is considered to be defending against the active-attack on the chain link. If the active-attack ceases to exist, last known information about the active-attack is used to determine the properties, control, and ownership of the attack.[1.2.3](#123)
...
<a id="722"></a>
### 7.2.2 First, at least one of the attack's attack-targets[1.4.5](#145) must still be a legal target. Otherwise, the Attack Step ends and the Close Step begins.[7.7](#77)
...
<a id="723"></a>
### 7.2.3 Second, each of the resolution abilities of the attack generates its effects in the order specified (except go again), and then the attack moves onto the combat chain as a chain link.[7.0.3](#703) Attacking objects on a chain link remain attacking until they leave the combat chain.
...
<a id="723d"></a>
#### 7.2.3d If the active-attack does not exist, or cannot move to the combat chain as a chain link, the Attack Step ends and the Close Step begins.[7.7](#77)
...
<a id="724a"></a>
#### 7.2.4a If there is an attack-source, and it is a living object[2.5.1a](#251a), it becomes attacking. Otherwise, the controller of the attack and their hero card become the "attacking hero" until the active chain link resolves or the combat chain closes. If an effect of a source on the chain link refers to the attacking hero, it refers to that hero until the combat chain closes.
...
<a id="732"></a>
### 7.3.2 First, defending cards are declared for the attack-target(s). Cards declared this way become defending cards for the attack-target(s) on the active chain link.[7.0.5](#705) Declaring a card this way is not considered playing that card - it does not incur the cost of playing that card, it does not add it as a layer on the stack, and it does not resolve any resolution abilities on that card.
...
<a id="732d"></a>
#### 7.3.2d All declared cards for an attack-target are put onto the active chain link as a single compound event as defending cards for that attack-target. The order in which the declared cards become defending within the compound event is determined by the order in which were declared.[1.9.2](#192)
...
<a id="732e"></a>
#### 7.3.2e If two or more players may declare defending cards for an attack-target, they do so in clockwise order starting with the player that controls the attack-target.[1.1.6](#116)
...
<a id="742a"></a>
#### 7.4.2a The player that controls the attack may play/activate attack reaction[8.1.2](#812) cards/abilities when they have priority during the Reaction Step.

<a id="742b"></a>
#### 7.4.2b A player that controls a hero as an attack-target (if any) may play/activate defense reaction[8.1.3](#813) cards/abilities when they have priority during the Reaction Step.
...
<a id="742d"></a>
#### 7.4.2d When a defense reaction card resolves it becomes a defending card on the active chain link for its controller's hero.[7.0.5](#705) A defense reaction card fails to resolve if it cannot become a defending card.[5.3.2c](#532c)
...
<a id="752a"></a>
#### 7.5.2a Dealing {p} damage to the attack-target this way, is a hit-event generated by the game.[7.5.5](#755) The source of the damage is the attack.
...
<a id="755"></a>
### 7.5.5 An attack is considered to have hit if it deals damage to an attack-target during the Damage Step of combat. The hit-event is otherwise identical to a deal {p} damage event[8.5.3](#853) and only occurs during the Damage Step of combat.
...
<a id="772a"></a>
#### 7.7.2a If all players pass in succession when the stack is empty during the Resolution Step, the Close Step begins.[7.6.4](#764)

<a id="772b"></a>
#### 7.7.2b If there are no valid attack-targets at the beginning of the Attack Step, the Close Step begins.[7.2.2](#722)

<a id="772c"></a>
#### 7.7.2c If the active-attack of the active chain link does not exist or cannot move to the combat chain as a chain link,[7.2.3d](#723d) or the active-attack ceases to exist before damage is calculated,[7.5.2](#752) the Close Step begins as a game state action.[1.10.2](#1102)
...
<a id="772d"></a>
#### 7.7.2d If an effect closes the combat chain, the Close Step begins as a game state action.[1.10.2](#1102)
...
<a id="774"></a>
### 7.7.4 Second, layers on the stack resolve and game state actions are performed as if all players are passing priority in succession.[1.10.2](#1102)
...
<a id="776"></a>
### 7.7.6 Fourth, all remaining objects on the combat chain are cleared.[3.0.12](#3012)
...
An action card is a deck-card.[1.3.2c](#132c)
<a id="811a"></a>
#### 8.1.1a An action card/activated ability can only be played/activated when the stack is empty.

<a id="811b"></a>
#### 8.1.1b An action card/activated ability cannot be played/activated during combat, except during the Resolution Step of combat.[7.0.1a](#701a)

<a id="811c"></a>
#### 8.1.1c An action card/activated ability has the additional asset-cost of one action point to play/activate.[5.1.6](#516)
...
An attack reaction card is a deck-card.[1.3.2c](#132c)
<a id="812a"></a>
#### 8.1.2a An attack reaction card/activated ability can only be played/activated by a player who controls the attack during the Reaction Step of combat.[7.4](#74)

<a id="812b"></a>
#### 8.1.2b When an attack reaction card resolves as a layer on the stack, it is cleared.[3.0.12](#3012)
...
A defense reaction card is a deck-card.[1.3.2c](#132c)
<a id="813a"></a>
#### 8.1.3a A defense reaction card/activated ability can only be played/activated by a player who controls a hero as an attack-target during the Reaction Step of combat.[7.4](#74)

<a id="813b"></a>
#### 8.1.3b When a defense reaction card resolves as a layer on the stack, it becomes a defending card on the active chain link.[7.0.5](#705)
...
An equipment card (without any other types) is an arena-card.[1.3.2d](#132d)
<a id="814a"></a>
#### 8.1.4a As an arena-card, a player may equip an equipment card from their card-pool during the start-of-game procedure to its respective zone.[4.1](#41)

<a id="814b"></a>
#### 8.1.4b An equipment permanent may be declared as a defending card during the Defend Step of combat.[7.3.2](#732)

<a id="815"></a>
### 8.1.5 Hero

A hero card is a hero-card.[1.3.2a](#132a)
<a id="815a"></a>
#### 8.1.5a A player starts the game with their hero card as a permanent in their hero zone.[4.1](#41)

<a id="815b"></a>
#### 8.1.5b A hero card is separate from, and cannot be included in a player's, card-pool.[1.1.3](#113)

<a id="816"></a>
### 8.1.6 Instant

An instant card is a deck-card.[1.3.2c](#132c)
<a id="816a"></a>
...
A resource card is a deck-card.[1.3.2c](#132c)
<a id="817a"></a>
...
A token card is a token-card.[1.3.2b](#132b)
<a id="818a"></a>
#### 8.1.8a Tokens only exist in the arena or as sub-cards.[3.0.14](#3014) If a token leaves the arena and it is not a sub-card, it ceases to exist.

<a id="819"></a>
### 8.1.9 Weapon

A weapon card is an arena-card.[1.3.2d](#132d)
<a id="819a"></a>
#### 8.1.9a As an arena-card, a player may equip a weapon card from their card-pool during the start-of-game procedure to its respective zone.[4.1](#41)

<a id="8110"></a>
### 8.1.10 Mentor

A mentor card is a deck-card.[1.3.2c](#132c)
<a id="8110a"></a>
...
A demi-hero card is an arena-card.[1.3.2d](#132d)
<a id="8111a"></a>
...
<a id="8111b"></a>
#### 8.1.11b If a demi-hero becomes a permanent in the arena and the controlling player does not control a hero, the demi-hero is considered to be that player's hero and has the hero type for the purposes of rules and effects for the rest of the game. Otherwise, if the player already controls a hero, the demi-hero is cleared from the arena.[3.0.12](#3012)

<a id="8112"></a>
### 8.1.12 Block

A block card is a deck-card.[1.3.2c](#132c)
<a id="8112a"></a>
...
<a id="8113a"></a>
#### 8.1.13a Only macro objects have the macro type.[1.5](#15)

<a id="8114"></a>
### 8.1.14 Companion

A companion card is an arena-card.[1.3.2d](#132d)
<a id="82"></a>
...
<a id="823a"></a>
#### 8.2.3a An attack card is considered an attack when on the stack or when it is on the combat chain as an attacking card.[1.4.2](#142)

<a id="823b"></a>
#### 8.2.3b When an attack card is played, the combat chain opens (if it is closed) and the layer step of combat begins.[7.1](#71)
...
<a id="828a"></a>
#### 8.2.8a If an ally permanent ceases to exist, it is considered to have died.[2.5.3g](#253g)

<a id="828b"></a>
#### 8.2.8b During the End Phase, an ally's life total is reset to its base life.[4.4](#44)

<a id="828c"></a>
#### 8.2.8c If an ally is attacking, the controlling player and their hero are not considered an attacking hero for that chain link.[7.2.4a](#724a)

<a id="828d"></a>
#### 8.2.8d If an ally is the target of an attack, the controlling player and their hero are not considered a defending hero for that chain link; and the player cannot declare defending cards or play or activate defense reaction cards or abilities during the reaction step of combat[7.2.4b](#724b)
...
<a id="829b"></a>
#### 8.2.9b When a landmark enters the arena, it becomes a permanent and all other landmark permanents are cleared, except when the landmark is added as a defending card to a chain link.[3.0.12](#3012)
...
<a id="8213a"></a>
#### 8.2.13a When an invocation resolves as a layer on the stack, it enters the arena with its back-face active and becomes a permanent.[9.1](#91)

<a id="8214"></a>
### 8.2.14 Construct

<a id="8214a"></a>
#### 8.2.14a When a construct resolves as a layer on the stack, it enters the arena with its back-face active and becomes a permanent.[9.1](#91)
...
<a id="831a"></a>
#### 8.3.1a When an attack-proxy resolves on the stack, it and its source move onto the combat chain as a chain link.[7.2.3b](#723b)

<a id="831b"></a>
#### 8.3.1b If attack-proxy is added to the stack, the combat chain opens (if it is closed) and the Layer Step of combat begins.[7.1](#71)

<a id="831c"></a>
#### 8.3.1c To add an attack-proxy onto the stack, a legal attackable target must be declared as the target of the attack.[1.4.5a](#145a)
...
<a id="834a"></a>
#### 8.3.4a If an attack with dominate is currently defended by a card from hand, an additional card cannot be added as a defending card to the attack's chain link if the card comes from a player's hand.[5.3.2c](#532c)[8.5.32](#8532)

<a id="834b"></a>
#### 8.3.4b If an attack with dominate is currently defended by a card from hand on the activate chain link, defense reaction cards cannot be played from hand.[7.4.2c](#742c)
...
<a id="835a"></a>
#### 8.3.5a If go again is an ability of a non-attack layer on the stack, the controlling player gains 1 action point after all other resolution abilities have resolved.[5.3.5](#535)

<a id="835b"></a>
#### 8.3.5b If go again is an ability of an attack on the active chain link, the controlling player gains 1 action point at the beginning of the Resolution Step of combat.[7.6.2](#762)
...
Specialization is a meta-static ability. Specialization is written as "[HERO] Specialization" which means "You may only have this in your deck if your hero is [HERO]," where HERO is the moniker of the player's hero card.[2.7.3](#273)
<a id="838"></a>
...
<a id="8313b"></a>
#### 8.3.13b If an attack is destroyed by phantasm before damage for its chain link has been calculated, the combat chain closes.[7.7.2](#772) If an attack is destroyed by phantasm after damage for its chain link has been calculated, the combat chain does not close.
...
<a id="8314a"></a>
#### 8.3.14a An object with Spectra can be the target of an attack, even if it is not a living object.[1.4.5a](#145a)

<a id="8314b"></a>
#### 8.3.14b When an object with Spectra becomes the target of an attack, a triggered-layer is put on the stack. When the triggered-layer resolves, the object with Spectra is destroyed. If there are not other legal attack-targets, the combat chain closes.[7.7.2](#772)
...
Essence is a meta-static ability. Essence is written in the format "Essence of [SUPERTYPES]" which means "You may have [SUPERTYPES] cards in your deck, as though your hero had those supertypes," where SUPERTYPES is a list of one or more supertypes.[2.11](#211)
<a id="8317"></a>
### 8.3.17 Fusion

Fusion is an optional additional-cost play-static ability. Fusion is written as "[SUPERTYPES] Fusion" which means "As an additional cost to play this, you may reveal (a/an) [SUPERTYPES] card(s) from your hand," where SUPERTYPES is a list of one or more supertypes.[2.11](#211)
<a id="8317a"></a>
...
<a id="8321a"></a>
#### 8.3.21a A player cannot include a card with Ephemeral in their card-pool.[1.1.3](#113)
...
<a id="8322a"></a>
#### 8.3.22a If an attack with overpower is currently defended by an action card, an additional action card cannot be added as a defending card to the attack's chain link..[8.5.32](#8532)
...
<a id="8338b"></a>
#### 8.3.38b Meld sets the asset-cost of a card before increases and decreases are applied. ([5.1.6](#516))

<a id="8338c"></a>
#### 8.3.38c A melded card has the properties of both of its sides on a split-card[9.2](#92): both names, and the combination of its abilities, supertypes, types, and subtypes. If melded card is not a split-card, it is unaffected by meld.
...
Combo is a label for a static ability typically written as "Combo - If [NAMES] was the last attack this combat chain, [EFFECTS]" where NAMES is a list of one or more names.[2.7](#27)
<a id="842"></a>
### 8.4.2 Crush

Crush is a label for a triggered-static ability typically written as "Crush - When this deals 4 or more damage, [EFFECTS]."
<a id="842a"></a>
#### 8.4.2a The Crush ability is conditional on an event that deals damage, not just a hit-event.[7.5.5](#755)
...
Channel is a label for an ability typically written as "Channel [SUPERTYPE] - At the beginning of your end phase, put a flow counter on this then destroy it unless you put a [SUPERTYPE] card from your pitch zone on the bottom of your deck for each flow counter on it." where SUPERTYPE is a supertype.[2.11](#211)
%
<a id="845"></a>
### 8.4.5 Material
...
Contract is a label for a static ability typically written as "Contract - You are contracted to [CONDITION]. Whenever you complete this contract, [EFFECT]" where CONDITION is one or more contract conditions, and EFFECT the reward for completing the contract.[8.5.39](#8539)
<a id="848"></a>
### 8.4.8 Surge
...
<a id="852a"></a>
#### 8.5.2a The properties of a created token, specified by the token keyword, are defined in .[8.6](#86)
...
<a id="853b"></a>
#### 8.5.3b There are 3 types of damage: generic, physical, and arcane. Generic damage is damage without a specified type - it is referred to as "damage." Physical damage is damage dealt by an object using its power value {p} during the damage step of combat - it is referred to as "{p} damage".[7.5](#75) Arcane damage is damage dealt by an effect that specifically deals arcane damage.

<a id="853c"></a>
#### 8.5.3c If a non-living object would be dealt damage, the effect fails.[2.5.1a](#251a)

<a id="853d"></a>
#### 8.5.3d Losing life because of a rule or effect other than dealing damage is not considered dealing damage. An effect that causes life loss is not considered damage.[8.5.12](#8512)
...
Gain is a discrete effect. To gain an asset, the player's or object's assets of the given type is increased by the specified amount.[1.13](#113)
<a id="857a"></a>
...
Gets is a continuous effect, referred to as modify, and includes all modifications to existing numerical properties. To modify an object, the numerical properties of the object are altered by the specified amount.[2.0.3](#203)
...
<a id="8510b"></a>
#### 8.5.10b If a player is not specified, the intimidate is a targeted effect.[1.8.5](#185)
...
Lose is a discrete effect. To lose an asset, the player's or object's assets of the given type is decreased by the specified amount.[1.13](#113)
<a id="8512a"></a>
...
<a id="8512b"></a>
#### 8.5.12b Losing life is not considered as damage being dealt, unless it is the result of a damage effect.[8.5.3](#853) Losing life is not considered as a hit, unless it is the result of damage being dealt by an attack during the damage step of combat.[7.5.5](#755)
...
Reveal is a discrete effect or continuous effect. To reveal a private object, make it public, then make it private again.[3.0.3](#303)
<a id="8517a"></a>
...
Name is a discrete effect. To name an object, declare the name property of an object.[2.7](#27)
<a id="8521a"></a>
...
Turn is a discrete effect. To turn a card face-up or face-down, that card becomes public (face-up) or private (face-down) respectively.[3.0.3](#303)
<a id="8524a"></a>
...
<a id="8525e"></a>
#### 8.5.25e If the reference has a copyable property with a value of *[1.12.3](#1123), the become/copy effect defines the value to be same as the corresponding printed property of the original object, or 0 if the object did not have that property.
...
Pay is a discrete effect. To pay an asset-cost, the player spends assets of the specified type and amount.[1.14.2](#1142)
<a id="8531a"></a>
...
Add is a discrete effect. To add a card to a chain link as a defending card, move it from its current zone onto that chain link as a defending card.[7.0.5](#705)
<a id="8532a"></a>
...
<a id="8534a"></a>
#### 8.5.34a The player that controls the effect is considered to have frozen an object, and the object is considered to be frozen. The object can be unfrozen by an effect.[8.5.37](#8537)
...
Transform is a discrete effect. To transform an object into a permanent, put the object under the permanent.[3.0.14](#3014)
<a id="8536a"></a>
...
<a id="8536c"></a>
#### 8.5.36c If the specified permanent is a token keyword, create that token, then put the transformed objects under it.[8.6](#86)
...
Unfreeze is a discrete effect. To unfreeze an object, all existing freeze effects no longer apply to that object.[8.5.34](#8534)
<a id="8537a"></a>
...
<a id="8538a"></a>
#### 8.5.38a A layer on the stack that can generate the attack effect is an attack-layer..[1.4.4](#144)

<a id="8538b"></a>
#### 8.5.38b When an attack-layer resolves on the stack, the specified object becomes an attack and chain link on the combat chain and effects that applied to the attack-layer apply to the object as an attack instead.[7.2.3c](#723c) If the object does not exist or cannot become an attack, the combat chain closes instead.[7.2.3d](#723d)

<a id="8538c"></a>
#### 8.5.38c If an attack-layer is added to the stack, the combat chain opens (if it is closed) and the Layer Step of combat begins.[7.1](#71)

<a id="8538d"></a>
#### 8.5.38d To add an attack-layer onto the stack, a legal attackable target must be declared as the target of the attack.[1.4.5a](#145a)
...
<a id="8541a"></a>
#### 8.5.41a If no zone is specified, the object may be equipped to any zone determined by its subtype.[3][2.10](#210)
...
<a id="8541d"></a>
#### 8.5.41d If the specified object is a token keyword,[8.6](#86) create that token, then equip it. If the token cannot be created or equipped, the effect fails.
...
Move is a discrete effect. To move a counter, remove the counter from its current object and put it onto the specified object.[8.5.14](#8514)[8.5.16](#8516)
<a id="8542a"></a>
...
Awaken is a discrete effect. To awaken a double-faced card, its back face becomes its active face.[9.1](#91)
<a id="8543a"></a>
...
Pitch is a discrete effect. To pitch a card, put it into it's owner's pitch zone and the player gains resources equal to its pitch value.[1.14.3](#1143)
<a id="8544a"></a>
...
<a id="8545b"></a>
#### 8.5.45b If a player's revealed card does not have the power property, or they do not reveal a card, any other revealed card with the power property is considered to have greater power.[1.8.7](#187)
...
Mark is a discrete effect. To mark a hero, that hero has the marked condition.[9.3](#93)
<a id="8551"></a>
### 8.5.51 Retrieve
...
<a id="912b"></a>
#### 9.1.2b If an effect instructs a player to name a card,[8.5.21](#8521) they may name one side of a double-faced card, but not both.
...
<a id="913"></a>
### 9.1.3 A flip-card is a double-faced card that has a front-face and a back-face, with one of the following printed subtypes on its front-face: figment, invocation, or construct. Only one face of a flip-card is active at any given point. The properties of a flip-card are defined by its active face. The types of its front-face determines if it is a hero-, token-, deck-, or arena-card.[4.2](#42)

<a id="913a"></a>
#### 9.1.3a If a flip-card is outside the game, its front-face is active.

<a id="913b"></a>
#### 9.1.3b If a flip-card is inside the game, its front-face is active until a rule or effect would make its back-face active. If the card would then become a new card, its front-face is active again.[3.0.9](#309)
...
<a id="914"></a>
### 9.1.4 A twin-card is a double-faced card that has two faces. One or both faces of a twin-card may be active at any given point. The properties of a twin-card are defined by its active face(s). The combination of the types of both its faces determines if it is a hero-, token-, deck-, or arena-card.[4.2](#42)
...
<a id="915"></a>
### 9.1.5 A transcend-card is a double-faced card that has a front-face and a back-face, with the front-face that has an ability that generates the transcend effect.[8.5.48](#8548) Only one face of a transcend-card is active at any given point. The properties of a transcend-card are defined by its active face. The types of its front-face determines if it is a deck- or arena-card.[4.2](#42)

<a id="915a"></a>
#### 9.1.5a If a transcend-card is outside the game, its front-face is active.

<a id="915b"></a>
#### 9.1.5b If a transcend-card is inside the game, its front-face is active until it transcends and makes its back-face active. Its back-face remains active even if it would become a new object.[3.0.9](#309)
...
<a id="922"></a>
### 9.2.2 A split-card is considered a single card with all the properties of both sides, except when put onto the stack.[9.2.3](#923)
...
<a id="922a"></a>
#### 9.2.2a A split-card can only be included in a player's card-pool if the supertypes of both sides are a subset of their hero's supertypes.[1.1.3](#113)

> A player can only include Comet Storm||Shock if their hero has the Lightning and Wizard supertypes.

<a id="922b"></a>
#### 9.2.2b If an effect instructs a player to name a card,[8.5.21](#8521) they may name one of the two split-card names, but not both.
...
<a id="923"></a>
### 9.2.3 When a split-card is put onto the stack, the player that puts it there chooses one of its two sides.[5.1.2c](#512c) For the remainder of its existence, it is considered to have only the name, typebox, and textbox of the chosen side. The other side's name, typebox, and textbox are considered to not exist.
...
<a id="932"></a>
### 9.3.2 A hero is marked if an effect causes them to be marked[8.5.50](#8550)
...
<a id="933"></a>
### 9.3.3 When a marked hero is hit by a source controlled by an opponent,[7.5.5](#755) the marked condition of that hero is removed as part of the hit event. The removal of the marked condition is not a game state-process or a triggered effect.
